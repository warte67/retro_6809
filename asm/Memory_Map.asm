...
MMU_1_SELECT          equ    $FE86    ; (Byte) Page Select for 8K Memory Bank 1
MMU_2_SELECT          equ    $FE87    ; (Byte) Page Select for 8K Memory Bank 2 (MMU_META_HANDLE)
MMU_BLOCKS_FREE       equ    $FE88    ; (Word) Number of 32-Byte Blocks Available for Allocation
MMU_BLOCKS_ALLOCATED  equ    $FE8A    ; (Word) Number of 32-Byte Blocks Currently Allocated
MMU_BLOCKS_FRAGGED    equ    $FE8C    ; (Word) Number of 32-Byte Blocks Currently Fragmented
MMU_ARG_1             equ    $FE8E    ; (Word)  Argument 1 for MMU Command
MMU_ARG_1_MSB         equ    $FE8E    ; (Byte)  Argument 1 Most Significant Byte for MMU Command
MMU_ARG_1_LSB         equ    $FE8F    ; (Byte)  Argument 1 Least Significant Byte for MMU Command
MMU_ARG_2             equ    $FE90    ; (Word)  Argument 2 for MMU Command
MMU_ARG_2_MSB         equ    $FE90    ; (Byte)  Argument 2 Most Significant Byte for MMU Command
MMU_ARG_2_LSB         equ    $FE91    ; (Byte)  Argument 2 Least Significant Byte for MMU Command
MMU_COMMAND           equ    $FE92    ; (Byte) Memory Management Unit Command:
                                      ...
                                      ; 
MMU_ERROR             equ    $FE93    ; (Byte) Memory Management Unit Error Code:
                                      ;    $00 = NO ERROR
                                      ;    $01 = OUT OF MEMORY
                                      ...
                                      ;    $FF = UNKNOWN ERROR
                                      ; 
MMU_META_HANDLE       equ    $FE94    ; (Word) Handle for the current allocation chain
MMU_META_STATUS       equ    $FE96    ; (Byte) Status Flags:
                                      ...
                                      ; 
MMU_META_DATA         equ    $FE97    ; (32-Bytes) Data Window for the Current Allocation
MMU_META_ROOT         equ    $FEB6    ; (Word) Root node of the current allocation       (Read Only)
MMU_META_PREV         equ    $FEB8    ; (Word) Previous node of the current allocation   (Read Only)
MMU_META_NEXT         equ    $FEBA    ; (Word) Next node of the current allocation       (Read Only)
MMU_RAW_INDEX         equ    $FEBC    ; (Word) Raw Index of the current memory node  (For Debugging)
                                      ; 
MMU_END               equ    $FEBD    ; End of Banked Memory Register Space
MMU_TOP               equ    $FEBE    ; Top of Banked Memory Register Space
; _______________________________________________________________________

HDW_RESERVED_DEVICE   equ    $FEBE    ; START: Reserved Register Space
HDW_REG_END           equ    $FFF0    ; 306 bytes reserved for future use.
; _______________________________________________________________________
